...
export class ApiService {
  ...
  public createTodo(todo: Todo): Observable<Todo>{
  	return this.http.post(API_URL + '/todos', todo).map(response => {
  ...
  public getTodoById(id: number): Observable<Todo>{
  	return this.http.get(API_URL + '/todos/' + id).map(response => {
  		return new Todo(response.json());
  	}).catch(this.handleError);
  }

  public deleteTodoById(id: number): Observable<null>{
  	return this.http.delete(API_URL + '/todos/' + id).map(response => null).catch(this.handleError);
  }
